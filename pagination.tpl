...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id=""><a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">&laquo;&nbsp;{l s='Previous'}</a></li>
		{else}
			<li id="" class="disabled"><span>&laquo;&nbsp;{l s='Previous'}</span></li>
		{/if}
		{if $start==3}
		...
		{section name=pagination start=$start loop=$stop+1 step=1}
			{if $p == $smarty.section.pagination.index}
				<li class="active"><span>{$p|escape:'htmlall':'UTF-8'}</span></li>
			{else}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id=""><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">{l s='Next'}&nbsp;&raquo;</a></li>
		{else}
			<li id="" class="disabled"><span>{l s='Next'}&nbsp;&raquo;</span></li>
		{/if}
		</ul>
	{/if}
