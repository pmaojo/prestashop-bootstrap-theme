...
<div class="margin-form">';
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'PRD':
					...
					if (!is_null($product->id))
						$this->_menu .= '<li class="divider-vertical"></li><li'.$selected.'><a href="'.$product->getLink().'">'.$product->name.'</a></li>'.PHP_EOL;
					break;
				...
				case 'CMS':
					...
					if (count($cms))
						$this->_menu .= '<li class="divider-vertical"></li><li'.$selected.'><a href="'.$cms[0]['link'].'">'.$cms[0]['meta_title'].'</a></li>'.PHP_EOL;
					break;
				...
				case 'CMS_CAT':
					...
					if (count($category))
					{
						$this->_menu .= '<li class="divider-vertical"></li><li><a href="'.$category->getLink().'" class="">'.$category->name.'</a>';
				...
				case 'MAN':
					...
					if (!is_null($manufacturer->id))
					{
						...
						$link = new Link;
						$this->_menu .= '<li class="divider-vertical"></li><li'.$selected.'><a href="'.$link->getManufacturerLink((int)$id, $manufacturer->link_rewrite).'">'.$manufacturer->name.'</a></li>'.PHP_EOL;
					}
					break;
				...
				case 'SUP':
					...
					{
						$link = new Link;
						$this->_menu .= '<li class="divider-vertical"></li><li'.$selected.'><a href="'.$link->getSupplierLink((int)$id, $supplier->link_rewrite).'">'.$supplier->name.'</a></li>'.PHP_EOL;
					}
					break;
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						$this->_menu .= '<li class="divider-vertical"></li><li><a href="'.$link[0]['link'].'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'>'.$link[0]['label'].'</a></li>'.PHP_EOL;
					}
					break;
			}
		}
	...
	private function getCategory($id_category, $id_lang = false, $id_shop = false)
	{
		...
		{
			if($category->level_depth > 1)
				$this->_menu .= '<li class="divider-vertical"></li>';
	...
	private function getCMSMenuItems($parent, $depth = 1, $id_lang = false)
	{
		...
		if (count($categories) || count($pages))
		{
			...
			{
				$this->_menu .= '<li class="divider-vertical"></li><li>';
			...
			foreach ($pages as $page)
			{
				...
				$this->_menu .= '<li class="divider-vertical"></li><li '.$selected.'>';
