...
<form class="navbar-form" role="search" action="{$link->getPageLink('search')}" id="searchbox">
	...
	<div id="search_block_top">
		<form method="get" action="{$link->getPageLink('search')}">
			<div class="input-append">
				...
				<input type="hidden" name="orderby" value="position" />
				<input type="hidden" name="orderway" value="desc" />
			    <input class="" id="appendedInputButton" type="text" name="search_query" value="{if isset($smarty.get.search_query)}{$smarty.get.search_query|htmlentities:$ENT_QUOTES:'utf-8'|stripslashes}{/if}" />
			    <button class="btn" type="submit">{l s='Search' mod='blocksearch'}</button>
		    </div>
		</form>
	</div>
</div>
