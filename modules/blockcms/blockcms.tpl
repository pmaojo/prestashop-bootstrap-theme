...
{if $block == 1}
	...
	{foreach from=$cms_titles key=cms_key item=cms_title}
		<div id="informations_block_left_{$cms_key}" class="block informations_block_left">
			<p class="panel-heading"><a href="{$cms_title.category_link}">{if !empty($cms_title.name)}{$cms_title.name}{else}{$cms_title.category_name}{/if}</a></p>
...
{else}
	<!-- MODULE Block footer -->
	<div class="block_various_links col-md-2" id="block_various_links_footer">
		<p class="panel-heading">{l s='Information' mod='blockcms'}</p>
		<ul>
